...
#define ORIENTATION_WINDOW_R         3    // минимальный радиус окна в рамках которого будет выбрана ориентация (в пикселях), R=3 => 5x5 окно
#define ORIENTATION_VOTES_PEAK_RATIO 0.8 // 0.8 => если гистограмма какого-то направления получила >= 80% от максимального числа голосов - она тоже победила
...
#define SMOOTHING_WEIGHT 0.5 // коэффициент сглаживания гистограмм
...
void phg::SIFT::buildPyramids(const cv::Mat &imgOrg, std::vector<cv::Mat> &gaussianPyramid, std::vector<cv::Mat> &DoGPyramid) {
    ...
    for (size_t octave = 0; octave < NOCTAVES; ++octave) {
        ...
        for (ptrdiff_t layer = 1; layer < OCTAVE_GAUSSIAN_IMAGES; ++layer) {
            ...
            // COMMENT: картинка с новой октавы на самом первом слое имеет в 2 раза большее размытие (за счёт уменьшения размера в 2 раза),
            // чем картинка на (s-3) слое предыдущей октавы; подставим эти знания в формулу выше и получим sigma для текущего слоя:
            double sigma = INITIAL_IMG_SIGMA * pow(2.0, octave) * sqrt(pow(k, 2.0 * layer) - 1.0);
    ...
    for (size_t octave = 0; octave < NOCTAVES; ++octave) {
        for (size_t layer = 0; layer < OCTAVE_GAUSSIAN_IMAGES; ++layer) {
            ...
            // спойлер: подумайте с чем должна визуально совпадать картинка из октавы? может быть с какой-то из картинок с предыдущей октавы? с какой? как их визуально сверить ведь они разного размера? 
        }
    }
...
namespace {
    ...
    void parabolaFitting_set_values(float x0, float x1, float x2, float &shift, float &value) {
        rassert((x1 >= x0 && x1 >= x2) || (x1 <= x0 && x1 <= x2), 12541241241241);
        float a = (x2-2.0f*x1+x0) / 2.0f;
        float b = x1 - x0 - a;
        shift = - b / (2.0f * a) - 1.0f;
                ...
                for (ptrdiff_t j = 1; j < cur.rows - 1; ++j) {
                    for (ptrdiff_t i = 1; i + 1 < cur.cols; ++i) {
                        ...
                        for (int dx = -1; dx <= 1 && (is_min || is_max); ++dx) {
                            // TODO проверить, является ли наш центр все еще экстремумом по сравнению с соседом DoGs[1+dz].at<float>(j+dy, i+dx) ? 
                            // (не забудьте учесть, что один из соседов - это мы сами)
                            neighbours[dx+1][dy+1][dz+1] = DoGs[1+dz].at<float>(j+dy, i+dx);
                            if (!(dz == 0 && dx == 0 && dy == 0)){
                                is_max = is_max && (DoGs[1+dz].at<float>(j+dy, i+dx) <= center);
                                is_min = is_min && (DoGs[1+dz].at<float>(j+dy, i+dx) >= center);
                            }
                        }
                        }
                        }
                        ...
                        float dy = 0.0f;
                        float dvalue = 0.0f;
                        // TODO сделать субпиксельное уточнение (хотя бы через параболу-фиттинг независимо по оси X и оси Y,
                        // но лучше через честный ряд Тейлора, матрицу Гессе и итеративное смещение,
                        // если экстремум оказался в соседнем пикселе)
                        ...
                        {
                            // TODO
                            float dValue_x, dValue_y;
                            // по оси X:
                            parabolaFitting_set_values(neighbours[0][1][1], neighbours[1][1][1], neighbours[2][1][1], dx, dValue_x);
                            // по оси Y:
                            parabolaFitting_set_values(neighbours[1][0][1], neighbours[1][1][1], neighbours[1][2][1], dy, dValue_y);
                            dvalue = dValue_x + dValue_y;
                        }
#endif
                        ...
                        for (size_t bin = 0; bin < ORIENTATION_NHISTS; ++bin) {
                            ...
                            if (value > prevValue && value > nextValue && votes[bin] > biggestVote * ORIENTATION_VOTES_PEAK_RATIO) {
                                ...
                                float bin_corr = parabolaFitting(prevValue, value, nextValue);
                                kp.angle = (bin_corr + 0.5) * (360.0 / ORIENTATION_NHISTS);
...
bool phg::SIFT::buildLocalOrientationHists(const cv::Mat &img, size_t i, size_t j, size_t radius,
    ...
    for (size_t y = j - radius + 1; y < j + radius; ++y) {
        for (size_t x = i - radius + 1; x < i + radius; ++x) {
            ...
            // ! индексы по x и y на изображении поменяны местами!
            double delta_x = img.at<float>(y, x+1) - img.at<float>(y, x-1);
            double delta_y = img.at<float>(y+1, x) - img.at<float>(y-1, x);
            double magnitude = delta_x * delta_x + delta_y * delta_y;

            // orientation == theta
            // atan( (L(x, y + 1) − L(x, y − 1)) / (L(x + 1, y) − L(x − 1, y)) )
            double orientation = atan2(delta_y, delta_x); // подсказка - используйте atan2(dy, dx)
            ...
            sum[(bin + 1)%ORIENTATION_NHISTS] += SMOOTHING_WEIGHT * magnitude;
            sum[(bin+ORIENTATION_NHISTS-1)%ORIENTATION_NHISTS] += SMOOTHING_WEIGHT * magnitude;
        }
    }
    ...
    for (int hstj = 0; hstj < DESCRIPTOR_SIZE; ++hstj) { // перебираем строку в решетке гистограмм
        for (int hsti = 0; hsti < DESCRIPTOR_SIZE; ++hsti) { // перебираем колонку в решетке гистограмм
                ...
                for (int smpi = 0; smpi < DESCRIPTOR_SAMPLES_N; ++smpi) { // перебираем столбик очередного замера для текущей гистограммы
                    for (int smpy = 0; smpy < smpW; ++smpy) { // перебираем ряд пикселей текущего замера
                        for (int smpx = 0; smpx < smpW; ++smpx) { // перебираем столбик пикселей текущего замера
                            ...
                            // Условие немного неправильное было; без дополнительной проверки на равенство тесты падают
                            ...
                            // TODO за счет чего этот вклад будет сравниваться с этим же вкладом,
                            // даже если эта картинка будет повернута? 
                            ...
                            sum[bin] += magnitude;
                            // TODO хорошая идея добавить трилинейную интерполяцию как предложено в статье,
                            // или хотя бы сэмулировать ее - сгладить получившиеся гистограммы
                            sum[(bin + 1)%ORIENTATION_NHISTS] += SMOOTHING_WEIGHT * magnitude;
                            sum[(bin+ORIENTATION_NHISTS-1)%ORIENTATION_NHISTS] += SMOOTHING_WEIGHT * magnitude;
                        }
                    }
            ...
            for (int bin = 0; bin < DESCRIPTOR_NBINS; ++bin) {
                vector_length += sum[bin] * sum[bin];
            }
            vector_length = sqrt(vector_length);
            for (int bin = 0; bin < DESCRIPTOR_NBINS; ++bin) {
                sum[bin] /= vector_length;
            }
