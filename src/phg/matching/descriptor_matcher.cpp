                                                   ...
                                                   std::vector<cv::DMatch> &filtered_matches)
{
    ...
    int j = 0;
    for (int i = 0; i < matches.size(); i++) {
        std::set<int> near_query_ids;
        std::set<int> near_train_ids;

        j = 0;
        while (near_query_ids.size() < total_neighbours && distances2_query.at<float>(i, j) < radius2_query) {
            near_query_ids.insert(indices_query.at<int>(i, j));
            j++;
        }
        j = 0;
        while (near_train_ids.size() < total_neighbours && distances2_train.at<float>(i, j) < radius2_train) {
            near_train_ids.insert(indices_train.at<int>(i, j));
            j++;
        }

        std::vector<int> intersection_ids;
        std::set_intersection(near_query_ids.begin(), near_query_ids.end(),
                              near_train_ids.begin(), near_train_ids.end(),
