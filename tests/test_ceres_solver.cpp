...
TEST (CeresSolver, HelloWorld1) {
    ...
    // Считаем производную функции f(x) = 10 - x, она равна -1.
...
TEST (CeresSolver, HelloWorld2) {
    ...
    for (int d = 0; d < 3; ++d) {
       EXPECT_NEAR(point[d], expected_point_solution[d], 1e-4);
        ...
        // Проблема была в подсчёте расстояния до параболоида (в строчке 136)
    }
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    for (int d = 0; d < 3; ++d) {
       ASSERT_NEAR(line_params[d] / line_params[2], ideal_line[d] / ideal_line[2], threshold);
        ...
        // Ответ: Проблема в том, что у прямой 2 степени свободы, а мы задаём её 3 коэффициентами.
        // Чтобы решить эту проблему, во-первых, можно "отнормировать" прямую, например, поделив коэффициенты на последний из них.
        // Во-вторых, можно изначально задавать прямые 2 коэффициентами: например, записать равенство для 2 точек, принадлежащих данной прямой.
    }
    ...
    // Ответ: При шумных данных слишком низкая вероятность того, что 99% точек окажутся инлаерами.
    ...
    // Ответ: Падало на тестах с очень шумными данными. Проблема в том, что evaluateLine было сравнение расстояния без модуля с 3*sigma,
    // поэтому для шумных точек с большим по модулю отрицательным расстоянием до прямой сравнение выполнялось => и они засчитывались как инлаеры
